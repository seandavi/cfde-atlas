...
- **SCHEMA SCOPE** — Documents the `analytics` schema search_path and the gap between what `BLUEPRINT.md` aspires to and what `list_tables` actually returns today. Removing this rule causes the model to invent tables. **(Amended by ADR 0006.)** When `buildSystemPrompt` receives a non-empty `schemaDigest`, this section is replaced by an `AVAILABLE SCHEMA` block that inlines the live column-level schema and declares it authoritative; the `TOOLS` discovery bullets (`list_tables` / `describe_table`) are correspondingly demoted to a stale-schema fallback (call `describe_table` only on an unknown-table/column error). The `SCHEMA SCOPE` wording above is retained verbatim as the fallback used when no digest is injected (introspection failure / the legacy `SYSTEM_PROMPT` export). User-visible effect: the model writes SQL directly without two discovery round-trips, lowering per-turn latency; behavior is unchanged when the digest is absent.
